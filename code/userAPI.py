...
class User_API:
    def __init__(self):
        # 载入数据
        self.user_login_data = json.loads(open('./user.json', 'r').read())
        self.token_data = json.loads(open('./token.json', 'r').read())
        # 清理过期token
        for it in list(self.token_data.keys()):
            if time.time() - self.token_data[it]['time'] > 60 * 60 * 24:
                del self.token_data[it]
        json_save(self.token_data, './token.json')
    ...
    def signup(self, userName, password):
        ...
        if userName in self.user_login_data:
            ...
            json_save(self.user_login_data, './user.json')
